...
The reasoning behind Reba, kept out of the README so that stays short.

Everything here explains *why* something is the way it is. If you are changing
the acuity test, the reflex thresholds, the storage rules or the consent text,
read the relevant section first: most of these choices were made against a
specific failure, and the failure is described.
